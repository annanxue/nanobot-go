...
function getAuthHeaders() {
    const token = window.NANOBOT_TOKEN || '';
    return token ? {'X-Nanobot-Token': token} : {};
}
...
function saveVisualConfig() {
    try {
        ...
        cfg.channels.mochat.enabled = false; // Default to disabled since UI is removed
        cfg.channels.mochat.baseUrl = '';
        cfg.channels.mochat.socketUrl = '';
...
function restartService() {
    ...
    if (statusEl) statusEl.textContent = '正在重启...';
    authedFetch('/api/restart', {method: 'POST'})
        .then(r => r.json())
        .then(d => {
            if (d.success) {
